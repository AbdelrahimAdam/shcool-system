<template>
  <aside 
    class="sidebar fixed top-16 left-0 w-72 h-full bg-gradient-to-b from-gray-900 to-gray-800 shadow-2xl transition-transform duration-300 ease-in-out z-40 overflow-y-auto"
  ...
  >
    ...
    <div class="p-5 border-b border-gray-700">
      <div class="flex items-center gap-3">
        ...
        <div>
          <span class="font-bold text-white text-lg">School Admin</span>
          <p class="text-xs text-gray-400 mt-0.5">Management Portal</p>
        </div>
      </div>
    </div>
    ...
    <nav class="py-6">
      <div class="px-4 space-y-1.5">
        ...
        >
          <!-- Dashboard Icon -->
          <svg v-if="item.icon === 'DashboardIcon'" class="w-5 h-5 mr-3 flex-shrink-0 transition-all duration-200" :class="$route.path === item.path ? 'text-yellow-400' : 'text-gray-500 group-hover:text-gray-300'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
          </svg>
          
          <!-- Students Icon -->
          <svg v-else-if="item.icon === 'StudentsIcon'" class="w-5 h-5 mr-3 flex-shrink-0 transition-all duration-200" :class="$route.path === item.path ? 'text-yellow-400' : 'text-gray-500 group-hover:text-gray-300'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
          </svg>
          
          <!-- Teachers Icon -->
          <svg v-else-if="item.icon === 'TeachersIcon'" class="w-5 h-5 mr-3 flex-shrink-0 transition-all duration-200" :class="$route.path === item.path ? 'text-yellow-400' : 'text-gray-500 group-hover:text-gray-300'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
          </svg>
          
          <!-- Classes Icon -->
          <svg v-else-if="item.icon === 'ClassesIcon'" class="w-5 h-5 mr-3 flex-shrink-0 transition-all duration-200" :class="$route.path === item.path ? 'text-yellow-400' : 'text-gray-500 group-hover:text-gray-300'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v11a2 2 0 01-2 2z" />
          </svg>
          
          <!-- Attendance Icon -->
          <svg v-else-if="item.icon === 'AttendanceIcon'" class="w-5 h-5 mr-3 flex-shrink-0 transition-all duration-200" :class="$route.path === item.path ? 'text-yellow-400' : 'text-gray-500 group-hover:text-gray-300'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          
          <!-- Payments Icon -->
          <svg v-else-if="item.icon === 'PaymentsIcon'" class="w-5 h-5 mr-3 flex-shrink-0 transition-all duration-200" :class="$route.path === item.path ? 'text-yellow-400' : 'text-gray-500 group-hover:text-gray-300'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          
          <!-- CRM Icon -->
          <svg v-else-if="item.icon === 'CRMIcon'" class="w-5 h-5 mr-3 flex-shrink-0 transition-all duration-200" :class="$route.path === item.path ? 'text-yellow-400' : 'text-gray-500 group-hover:text-gray-300'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <!-- Active indicator -->
          <div v-if="$route.path === item.path || ($route.path.startsWith(item.path) && item.path !== '/admin')" class="w-1 h-8 bg-gradient-to-b from-yellow-400 to-yellow-500 rounded-full"></div>
        </router-link>
      </div>
    </nav>
