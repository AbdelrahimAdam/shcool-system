<template>
  <div class="file-uploader">
    ...
    >
      ...
      <div v-if="!previewUrl">
        ...
        <p class="mt-2 text-sm text-gray-600">
          {{ languageStore.t('clickOrDrag') }}
        </p>
        <p class="text-xs text-gray-500 mt-1">
          {{ languageStore.t('acceptedFileTypes') }} {{ acceptedTypes }} (Max {{ maxSize }}MB)
        </p>
      </div>
    ...
    <div v-if="uploadProgress > 0 && uploadProgress < 100" class="mt-2">
      ...
      <p class="text-xs text-gray-500 mt-1">{{ uploadProgress }}% {{ languageStore.t('uploaded') }}</p>
    </div>
