<template>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">{{ languageStore.t('paymentApproval') }}</h1>
        <p class="text-sm text-gray-500 mt-1">{{ languageStore.t('reviewAndApprovePayments') }}</p>
      </div>
      <div class="flex items-center gap-2">
        <span class="badge-warning px-3 py-1">{{ pendingCount }} {{ languageStore.t('pending') }}</span>
      </div>
    </div>
...
const pendingPayments = computed(() => {
  return paymentStore.payments.filter(p => p.status === 'pending')
})
...
const approvePayment = async (paymentId) => {
  ...
  try {
    ...
    alert(languageStore.t('approveError') || 'Error approving payment')
  } finally {
...
const rejectPayment = async (paymentId) => {
  ...
  try {
    ...
    alert(languageStore.t('rejectError') || 'Error rejecting payment')
  } finally {
    isProcessing.value = null
  }
}
